...
int renderingMirror = 0;
int turnOn2dDrawing = 0;
static int drawingSprite = 0;
static int drawingPara = 0;
static int transparentParascan = 0;
int viewTileNumber = 0;

static float static_global_s1;
static float static_global_s2;
static float static_global_t1;
static float static_global_t2;

extern int noWallFog;
extern int use512x384;

extern float global_s1;
extern float global_s2;
extern float global_t1;
extern float global_t2;
...
extern float aspectToTexCoordFactors[][2];

extern int _wp1;
extern int _wp2;

void drawGlideQuad(long, long, long, long, long, long, long, long, float, float, float, float, float, float, float, float);
int setglidepic(short picnum, int mask);
void setupSpriteS1S2T1T2TexCoords(long bx, long xv, long by, long yv, long cnt, short picnum);
...
ceilscan (long x1, long x2, long sectnum)
{
	...
	if (!setglidepic(globalpicnum, 0)) return;
	...
	else
	{
		...
		i = ksqrt(ox*ox+oy*oy); if (i == 0) i = 1024; else i = 1048576/i;
...
wallscan(long x1, long x2, short *uwal, short *dwal, long *swal, long *lwal)
{
	...
	long previous_vplce = 0, previous_vince = 0;
	...
	setupS1S2TexCoordsStaticStuff();
	setupT1T2TexCoordsStaticStuff();
	...
	for(x=x1;x<=x2;x++)
	{
		...
		if (noWallFog)
			palookupoffse[0] = fpalookup+(getpalookup(globalshade,globalshade)<<8);
		else
		...
		global_s1 *= vplce[0];
		global_s2 *= vince[0];
		global_s2 *= y2ve[0]-y1ve[0]-1;
		global_s2 += global_s1;

		convertAndDownloadPaletteLookup(palookupoffse[0]);
		...
		drawGlideLineVert(x,y1ve[0],x,y2ve[0]);
...
maskwallscan(long x1, long x2, short *uwal, short *dwal, long *swal, long *lwal)
{
	...
	long myx1 = 0, myy1 = 0, myx2 = 0, myy2 = 0, myx3 = 0, myy3 = 0, myx4 = 0, myy4 = 0;
	float mys1 = 0, myt1 = 0, mys2 = 0, myt2 = 0, mys3 = 0, myt3 = 0, mys4 = 0, myt4 = 0;
	...
	if (!setglidepic(globalpicnum, 1)) return;
	...
	if (drawingSprite)
	{
		...
		x = startx;
		holdy1 = max(uwal[x],startumost[x+windowx1]-windowy1);
		holdy2 = min(dwal[x],startdmost[x+windowx1]-windowy1);
		
		for(x=startx;x<=x2;x++)
		{
			y1 = max(uwal[x],startumost[x+windowx1]-windowy1);
			y2 = min(dwal[x],startdmost[x+windowx1]-windowy1);
	...
	if (drawingSprite)
	{
		...
		palookupoffse[0] = fpalookup+(getpalookup((long)mulscale16(swal[x],globvis),globalshade)<<8);
		bufplce[0] = lwal[x] + globalxpanning;
		setupT1T2TexCoords(bufplce[0]);
		if (bufplce[0] >= tsizx) { if (xnice == 0) bufplce[0] %= tsizx; else bufplce[0] &= tsizx; }
		if (ynice == 0) bufplce[0] *= tsizy; else bufplce[0] <<= tsizy;
		vince[0] = swal[x]*globalyscale;
		vplce[0] = globalzd + vince[0]*(y1ve[0]-globalhoriz+1);
		setupS1S2TexCoords(vplce[0], vince[0], y2ve[0]-y1ve[0]-1);
		convertAndDownloadPaletteLookup(palookupoffse[0]);
...
convertAndDownloadPaletteLookup(palookupoffse[0]);
		...
		if (mys1 != mys2 || mys3 != mys4)

			drawingSprite = 0;
		else
		{
			drawGlideQuad(myx1, myy1-1, myx2, myy2-1, myx3, myy3, myx4, myy4,
				mys1, myt1, mys2, myt2, mys3, myt3, mys4, myt4);
...
setgamemode(char davidoption, long daxdim, long daydim)
{
	...
	setview(1,1,xdim,ydim);
	setview(0L,0L,xdim,ydim);
	if (!setGlideWindow(xdim, ydim)) return -1;
...
hline (long xr, long yp)
{
	...
	if (noWallFog)
		shade = getpalookup(globalshade,globalshade)<<8;
	else
		shade = getpalookup((long)mulscale16(r,globvis),globalshade)<<8;

	hlineasm4(xr-xl,0L,(long)shade,
	...
	drawGlideLineHoriz(xr+1,yp,xl-1,yp);
...
transmaskvline (long x)
{
	...
	setupT1T2TexCoords(i);

	bufplc = waloff[globalpicnum]+i*tilesizy[globalpicnum];
	p = ylookup[y1v]+x+frameoffset;


	setupS1S2TexCoords(vplc, vinc, y2v-y1v);
	convertAndDownloadPaletteLookup(palookupoffs);

	tvlineasm1(vinc,palookupoffs,y2v-y1v,vplc,bufplc,p);
	
	drawGlideLineVert(x,y1v,x,y2v);
		...
		case 200:
			for(i=permtail;i!=permhead;i=((i+1)&(MAXPERMS-1)))
			{
				...
				if ((per->pagesleft > 0) && (per->pagesleft <= numpages))
					...
					dorotatesprite3dfx(per->sx,per->sy,per->z,per->a,per->picnum,
...
loadtile (short tilenume)
{
	...
	if (waloff[tilenume] == 0)
	{
		...
		allocache(&waloff[tilenume],dasiz*2,&walock[tilenume]);
...
screencapture(char *filename, char inverseit)
{
	...
	capturecount++;
	return glideDumpScreen(filename);
				  ...
				  long tilenume2, long sx2, long sy2)
{
	...
	if ((xsiz1 > 0) && (ysiz1 > 0) && (xsiz2 > 0) && (ysiz2 > 0))
	{
		...
		if (!setglidepic(tilenume1, 1)) return;

		if (waloff[tilenume2] == 0) loadtile(tilenume2);

		if (!setglidepic(tilenume2, 1)) return;
	...
	*hitx = hitscangoalx; *hity = hitscangoaly;
	...
	do
	{
		...
		if (sec->ceilingstat&2)
		{
			...
			i = ksqrt(dax*dax+day*day); if (i == 0) continue;
...
drawline256 (long x1, long y1, long x2, long y2, char col)
{
	...
	drawGlideOverheadLine(x1, y1, x2, y2, col);
	return;
...
printext256(long xpos, long ypos, short col, short backcol, char name[82], char fontsize)
{
	...
	glideWriteString(xpos, ypos, name);
...
rotatesprite (long sx, long sy, long z, short a, short picnum, signed char dashade, char dapalnum, char dastat, long cx1, long cy1, long cx2, long cy2)
{
	...
	if (((dastat&128) == 0) || (numpages < 2) || (beforedrawrooms != 0))
		...
		dorotatesprite3dfx(sx,sy,z,a,picnum,dashade,dapalnum,dastat,cx1,cy1,cx2,cy2);
...
rotatesprite3dfx (long sx, long sy, long z, short a, short picnum, signed char dashade, char dapalnum, char dastat, long cx1, long cy1, long cx2, long cy2)
{
	long i;
	permfifotype *per, *per2;
...
dorotatesprite (long sx, long sy, long z, short a, short picnum, signed char dashade, char dapalnum, char dastat, long cx1, long cy1, long cx2, long cy2)
{
	...
	if (!setglidepic(picnum, 1)) return;
	...
	setupSpriteS1S2T1T2TexCoordsStaticStuff(picnum);
	...
	for(x=x1;x<x2;x++)
	{
		...
		setupSpriteS1S2T1T2TexCoords(bx, xv, by, yv, y2-y1-1, picnum);
		drawGlideLineVert(x, y1-1, x, y2);
...
dorotatesprite3dfx (long sx, long sy, long z, short a, short picnum, signed char dashade, char dapalnum, char dastat, long cx1, long cy1, long cx2, long cy2)
{
	long cosang, sinang, v, nextv, dax1, dax2, oy, bx, by, ny1, ny2;
	long i, x, y, x1, y1, x2, y2, gx1, gy1, p, bufplc, palookupoffs;
	long xsiz, ysiz, xoff, yoff, npoints, yplc, yinc, lx, rx, xx, xend;
...

	{
		...
		drawingSprite = 1;
		x = x1;
		holdy1 = max(uplc[x],startumost[x]);
		holdy2 = min(dplc[x],startdmost[x]);
		for(x=x1;x<x2;x++)
		{
			y1 = max(uplc[x],startumost[x]);
			y2 = min(dplc[x],startdmost[x]);
			if (y1 != holdy1 || y2 != holdy2)
			{
				drawingSprite = 0;
				break;
	...
	if (drawingSprite)
	{
		...
		myy1 = uplc[x1]; myy3 = dplc[x1];
		if (!(dastat&8))
		{
			if (startumost[x1] > myy1) myy1 = startumost[x1];
			if (startdmost[x1] < myy3) myy3 = startdmost[x1];
		}
		myx1 = x1;
		myx3 = x1;
		...
		setupSpriteS1S2T1T2TexCoords(mybx, xv, myby, yv, myy3 - myy1 - 1, picnum);
		mys1 = global_s1;
		myt1 = global_t1;
		mys3 = global_s2;
		myt3 = global_t2;
		...
		myy2 = uplc[x2-1]; myy4 = dplc[x2-1];
		if (!(dastat&8))
		{
			if (startumost[x2-1] > myy2) myy2 = startumost[x2-1];
			if (startdmost[x2-1] < myy4) myy4 = startdmost[x2-1];
		}
		myx2 = x2-1;
		myx4 = x2-1;
		
		setupSpriteS1S2T1T2TexCoords(mybx, xv, myby, yv, myy4 - myy2 - 1, picnum);
...
setbrightness(char dabrightness, char *dapal)
{
	...
	convertAndDownloadPalette(tempbuf);
	...
	for(s=0,sec=&sector[s];s<numsectors;s++,sec++)
		if (show2dsector[s>>3]&pow2char[s&7])
		{
			...
			else
			{
				...
				i = ksqrt(ox*ox+oy*oy); if (i == 0) continue;
				i = 1048576/i;
...
getceilzofslope(short sectnum, long dax, long day)
{
	...
	i = (ksqrt(dx*dx+dy*dy)<<5); if (i == 0) return(sector[sectnum].ceilingz);
...
getflorzofslope(short sectnum, long dax, long day)
{
	...
	wal = &wall[sector[sectnum].wallptr];
	dx = wall[wal->point2].x-wal->x; dy = wall[wal->point2].y-wal->y;
	i = (ksqrt(dx*dx+dy*dy)<<5); if (i == 0) return(sector[sectnum].floorz);
	j = dmulscale3(dx,day-wal->y,-dy,dax-wal->x);
...
getzsofslope(short sectnum, long dax, long day, long *ceilz, long *florz)
{
	...
	*ceilz = sec->ceilingz; *florz = sec->floorz;
	if ((sec->ceilingstat|sec->floorstat)&2)
	{
		...
		i = (ksqrt(dx*dx+dy*dy)<<5); if (i == 0) return;
		j = dmulscale3(dx,day-wal->y,-dy,dax-wal->x);
...
wallmost(short *mostbuf, long w, long sectnum, char dastat)
{
	...
	dasqr = krecip(ksqrt(dx*dx+dy*dy));
...
grouscan (long dax1, long dax2, long sectnum, char dastat)
{
	...
	dasqr = krecip(ksqrt(wx*wx+wy*wy));
	...
	convertAndDownloadPaletteLookup(j);
	...
	for(x=dax1;x<=dax2;x++)
	{
		...
		if (y1 <= y2)
		{
...

			{
				...
				pal = fpalookup + (getpalookup(globalshade, globalshade)<<8);
				
				convertAndDownloadPaletteLookup(pal);
				drawGlideLineVert(x, y1, x, y2+1);
				global_oow1 = global_oow2 = 1;
			}
...
setglidepic(short picnum, int mask)
{
	return setGlideTexInfo(tilesizx[picnum], tilesizy[picnum], waloff[picnum], picnum, mask);
...
void setupS1S2TexCoordsStaticStuff()
{
	static_global_s1 = 1;

	static_global_s1 /= 1 << globalshiftval;
	static_global_s2 = static_global_s1;
...
void setupS1S2TexCoords(long vplc, long vinc, long cnt)
{
	global_s1 = static_global_s1;
	global_s2 = static_global_s2;
	global_s1 *= vplc;
...
void setupT1T2TexCoordsStaticStuff()
{
	static_global_t1 = 1;
...
void setupT1T2TexCoords(long bufplc)
{
	global_t1 = static_global_t1;
	global_t1 *= bufplc;
	global_t2 = global_t1;
...
void setupSpriteS1S2T1T2TexCoordsStaticStuff(short picnum)
{
	static_global_t1 = 1.0/65536.0;
	static_global_t2 = 1.0/65536.0;
	...
	{
		static_global_t1 /= tilesizx[picnum];
	}
	static_global_t1 *= aspectToTexCoordFactors[current_aspect_ratio][1];
	...
	{
		static_global_t2 /= tilesizx[picnum];
	}
	static_global_t2 *= aspectToTexCoordFactors[current_aspect_ratio][1];

	static_global_s1 = 1.0 / 65536.0;
	static_global_s2 = 1.0 / 65536.0;
	...
	{
		static_global_s1 /= tilesizy[picnum];
	}
	static_global_s1 *= aspectToTexCoordFactors[current_aspect_ratio][0];
	...
	{
		static_global_s2 /= tilesizy[picnum];
	}
	static_global_s2 *= aspectToTexCoordFactors[current_aspect_ratio][0];
}
